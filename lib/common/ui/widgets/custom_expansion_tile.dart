...
//Forked from original Flutter lib, updated for this project.

import 'package:flutter/material.dart';
import 'package:flutter/rendering.dart';
import 'package:ulastir/common/data/models/station_result.dart';
import 'package:ulastir/common/ui/widgets/route_navigation_grid_widget.dart';

const Duration _kExpand = Duration(milliseconds: 200);

/// Enables control over a single [RouteNavigationExpansionTile]'s expanded/collapsed state.
///
/// It can be useful to expand or collapse an [RouteNavigationExpansionTile]
/// programatically, for example to reconfigure an existing expansion
/// tile based on a system event. To do so, create an [RouteNavigationExpansionTile]
...
/// the [RouteNavigationExpansionTile] to rebuild, so they may not be called from
/// a build method.
class ExpansionTileController {
  /// Create a controller to be used with [RouteNavigationExpansionTile.controller].
  ExpansionTileController();

  _ExpansionTileState? _state;

  /// Whether the [RouteNavigationExpansionTile] built with this controller is in expanded state.
  ///
  ...
  /// See also:
  ///
  ///  * [expand], which expands the [RouteNavigationExpansionTile].
  ///  * [collapse], which collapses the [RouteNavigationExpansionTile].
  ///  * [RouteNavigationExpansionTile.controller] to create an RouteNavigationExpansionTile with a controller.
  bool get isExpanded {
    assert(_state != null);
    return _state!._isExpanded;
  }

  /// Expands the [RouteNavigationExpansionTile] that was built with this controller;
  ///
  ...
  ///
  /// Calling this method may cause the [RouteNavigationExpansionTile] to rebuild, so it may
  /// not be called from a build method.
  ///
  /// Calling this method will trigger an [RouteNavigationExpansionTile.onExpansionChanged] callback.
  ///
  /// See also:
  ///
  ...
  /// Collapses the [RouteNavigationExpansionTile] that was built with this controller.
  ///
  ...
  /// Finds the [ExpansionTileController] for the closest [RouteNavigationExpansionTile] instance
  /// that encloses the given context.
  ///
  /// If no [RouteNavigationExpansionTile] encloses the given context, calling this
  /// method will cause an assert in debug mode, and throw an
  /// exception in release mode.
  ///
  /// To return null if there is no [RouteNavigationExpansionTile] use [maybeOf] instead.
  ///
  /// {@tool dartpad}
  /// Typical usage of the [ExpansionTileController.of] function is to call it from within the
  /// `build` method of a descendant of an [RouteNavigationExpansionTile].
  ///
  /// When the [RouteNavigationExpansionTile] is actually created in the same `build`
  ...
  /// [BuildContext] that is "under" the [RouteNavigationExpansionTile]:
  ///
  ...
  /// would have an outer widget that creates the [RouteNavigationExpansionTile]
  ...
  static ExpansionTileController of(BuildContext context) {
    ...
    throw FlutterError.fromParts(<DiagnosticsNode>[
      ErrorSummary(
        'ExpansionTileController.of() called with a context that does not contain a RouteNavigationExpansionTile.',
      ),
      ErrorDescription(
        'No RouteNavigationExpansionTile ancestor could be found starting from the context that was passed to ExpansionTileController.of(). '
        'This usually happens when the context provided is from the same StatefulWidget as that '
        'whose build function actually creates the RouteNavigationExpansionTile widget being sought.',
      ),
      ErrorHint(
        'There are several ways to avoid this problem. The simplest is to use a Builder to get a '
        'context that is "under" the RouteNavigationExpansionTile. For an example of this, please see the '
        'documentation for ExpansionTileController.of():\n'
        '  https://api.flutter.dev/flutter/material/RouteNavigationExpansionTile/of.html',
      ),
      ErrorHint(
        'A more efficient solution is to split your build function into several widgets. This '
        'introduces a new context from which you can obtain the RouteNavigationExpansionTile. In this solution, '
        'you would have an outer widget that creates the RouteNavigationExpansionTile populated by instances of '
        'your new inner widgets, and then in these inner widgets you would use ExpansionTileController.of().\n'
        'An other solution is assign a GlobalKey to the RouteNavigationExpansionTile, '
        'then use the key.currentState property to obtain the RouteNavigationExpansionTile rather than '
  ...
  /// Finds the [RouteNavigationExpansionTile] from the closest instance of this class that
  /// encloses the given context and returns its [ExpansionTileController].
  ///
  /// If no [RouteNavigationExpansionTile] encloses the given context then return null.
  /// To throw an exception instead, use [of] instead of this function.
  ///
  /// See also:
  ///
  ///  * [of], a similar function to this one that throws if no [RouteNavigationExpansionTile]
...
/// [RouteNavigationExpansionTile] to save and restore its expanded state when it is scrolled
/// in and out of view.
///
...
/// to the [leading] and [trailing] properties of [RouteNavigationExpansionTile].
///
/// {@tool dartpad}
/// This example demonstrates how the [RouteNavigationExpansionTile] icon's location and appearance
/// can be customized.
///
...
/// programatically expand or collapse an [RouteNavigationExpansionTile].
///
/// ** See code in examples/api/lib/material/expansion_tile/expansion_tile.1.dart **
/// {@end-tool}
///
...
class RouteNavigationExpansionTile extends StatefulWidget {
  final Color titleColor;

  /// Creates a single-line [ListTile] with an expansion arrow icon that expands or collapses
  /// the tile to reveal or hide the [children]. The [initiallyExpanded] property must
  /// be non-null.
  const RouteNavigationExpansionTile({
    super.key,
    this.leading,
    required this.stationResult,
    required this.title,
  ...
  @override
  State<RouteNavigationExpansionTile> createState() => _ExpansionTileState();
}

class _ExpansionTileState extends State<RouteNavigationExpansionTile>
  ...
  Widget _buildChildren(BuildContext context, Widget? child) {
    ...
    return Container(
      ...
      /*decoration: BoxDecoration(
        ...
        color: widget.titleColor, //(widget.color ?? Colors.black87),
      ),*/
      child: Column(
        ...
        children: <Widget>[
          Semantics(
            hint: semanticsHint,
            onTapHint: onTapHint,
            child: LineTitleWidget(
                expanded: _isExpanded,
                onTap: () {
                  _handleTap();
                },
                active: widget.stationResult.active,
                line: widget.stationResult.routeObj?.type,
                title: widget.stationResult.routeObj?.lineNameShort ?? '',
                color: widget.stationResult.active
                    ? Colors.indigo[600]
                    : Colors.grey[700]),
  ...
  @override
  void didUpdateWidget(covariant RouteNavigationExpansionTile oldWidget) {
...
// BEGIN GENERATED TOKEN PROPERTIES - RouteNavigationExpansionTile
...
// END GENERATED TOKEN PROPERTIES - RouteNavigationExpansionTile
