...
import 'package:ulastir/common/data/models/api/bus_direction.dart';
import 'package:ulastir/common/data/models/api/bus_line.dart';
import 'package:ulastir/common/data/models/api/bus_station.dart';
...
class LineChoiceNotifier extends AutoDisposeNotifier<BusLine> {
  @override
  BusLine build() => BusLine();

  void update(BusLine line) => state = line;
}

final lineChoiceProvider = NotifierProvider.autoDispose<LineChoiceNotifier, BusLine>(
  LineChoiceNotifier.new,
);

class DirectionChoiceNotifier extends AutoDisposeNotifier<BusDirection> {
  @override
  BusDirection build() => BusDirection();

  void update(BusDirection direction) => state = direction;
}

final directionChoiceProvider =
    NotifierProvider.autoDispose<DirectionChoiceNotifier, BusDirection>(
  DirectionChoiceNotifier.new,
);

class StationChoiceNotifier extends Notifier<BusStation> {
  @override
  BusStation build() => BusStation();

  void update(BusStation station) => state = station;
}

final stationChoiceProvider = NotifierProvider<StationChoiceNotifier, BusStation>(
...
class _AddBusLinePageState extends ConsumerState<AddBusLinePage> {
  ...
  void _checkAndClearDirection() {
    ...
    if (direction.data != null) {
      ref.read(directionChoiceProvider.notifier).update(BusDirection());
    }
  }
  ...
  void _checkAndClearStation() {
    ...
    if (station.id != null) {
      ref.read(stationChoiceProvider.notifier).update(BusStation());
    }
  }
